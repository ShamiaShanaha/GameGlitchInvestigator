...
I decided whether a bug was really fixed is by running the app and playing with it to see if it was truly fixed. One test I ran is the pytest where I checked the check_guess function. For example, I tested guessing 60 when the secret was 50 and it gave me that it passed. This showed that the bug fix worked correctly and the logic behaved as expected. AI helped me design better tests by showing me that my function returns a tuple instead of a single string.
...
Streamlit reruns means the app rebuilds itself from scratch every time you do something, like clicking a button or entering a guess. So instead of only updating one small part of the screen, it basically replays the whole program again from top to bottom.
...
Next time I work with AI I will verify any suggestions instead of applying them immediately. Even if the AI sounds confident, I would always test the change before assuming it is correct. This project changed how I think about AI generated code because I realized it can be very helpful for spotting patterns and fixing bugs, but it still requires human testing and validation to make sure the final logic actually works correctly.
